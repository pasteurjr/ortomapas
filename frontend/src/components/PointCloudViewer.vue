...
const settingsKey = 'ortomapas.viewer3d.settings'
function saveSettings () { localStorage.setItem(settingsKey, JSON.stringify({ pointSize: pointSize.value, opacity: opacity.value, colorMode: colorMode.value, showGrid: showGrid.value, showAxes: showAxes.value, sampleLimit: sampleLimit.value })) }
function restoreSettings () { try { const value = JSON.parse(localStorage.getItem(settingsKey) || '{}'); if (value.pointSize) pointSize.value = value.pointSize; if (value.opacity) opacity.value = value.opacity; if (value.colorMode) colorMode.value = value.colorMode; if (typeof value.showGrid === 'boolean') showGrid.value = value.showGrid; if (typeof value.showAxes === 'boolean') showAxes.value = value.showAxes; if (value.sampleLimit) sampleLimit.value = value.sampleLimit } catch {} }
