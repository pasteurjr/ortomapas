  ...
  <section v-if="projectStore.activeProject" class="odm-tasks">
    ...
    <div v-for="task in tasks" :key="task.id" class="task-row">
      ...
      <div class="task-actions">
        ...
        <Button v-if="task.status === 'concluido' && surfaceFor(task)" label="DSM 3D" icon="pi pi-chart-line" size="small" severity="warn" @click="selectedSurface = surfaceFor(task)" />
...
function surfaceFor(task) { return products.value.find((p) => p.processamento_id === task.id && p.tipo === 'dsm') }
</script>
<style scoped>
